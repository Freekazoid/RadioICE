    ...
    <div class="icehot">
      <div class="newItemsTrack" v-for="item in newItems" :key="item.id">
        <div class="newItemsTrack-data">
          <div class="number" :class="item.play ? 'playTrack' : ''">{{ item.id }}</div>
...
export default {
  ...
  computed: {
    ...mapGetters({
      iceHot: 'IceHot/GET_NEW_ITEM',
      swip: 'NewItem/GET_SWIPER',
      swiperOP: 'NewItem/GET_SWIPER_OP',
    }),
  },
  methods: {
    playTrack(select){
