    ...
    <Swiper class="swiperHead" :options="swiperHeadOp" :auto-update="true" ref="swiperHead" dir="rtl">
      <Swiper-Slide v-for="item in swiperHead" :key="item.id">
        <router-link :to="item.linck" v-if="item.selfLinck">
          <GetImages :img="`headerSlider/${item.img}`"/>
        </router-link>
    ...
    <div class="radio">
          ...
          <Swiper-Slide v-for="item in swiperNewItems" :key="item.id" class="newTrack">
            ...
            <div class="new-block" :class="{'activeNewItems': item.hover}" @click="activeSlideNewItems(item)" @mouseenter="activeSlideNewItems(item)">
              <div class="imagesBlock">
                ...
                <div v-lazy:background-image="require(`../assets/img/newItams/${item.img}`)" class="images" @click="playTrack(item, 'swiperNewItems')">
      ...
      <div class="conteiner">
        <div class="icehot">
          ...
          <div class="newItemsTrack" v-for="item in newItems" :key="item.id">
            <div class="newItemsTrack-data">
              <div class="number" :class="{'playTrack': item.play}">{{ item.id }}</div>
        ...
        <div class="newVideo">
          ...
          <Swiper :options="swiperNewVideoOp" ref="swiperNewVideo" class="videoSwiper">
            <Swiper-Slide v-for="item in swiperNewVideo" :key="item.id">
...
export default {
  ...
  computed: {
    ...mapGetters({
      itemNew: 'Radio/GET_NEW_ITEM',
      swHead: 'Radio/GET_SWIPER_HEAD',
      swHeadOp: 'Radio/GET_SWIPER_HEAD_OP',
      swItem: 'Radio/GET_SWIPER_NEW_ITEM',
      swItemOp: 'Radio/GET_SWIPER_NEW_ITEM_OP',
      video: 'Radio/GET_SWIPER_NEW_VIDEO',
      videoOp: 'Radio/GET_SWIPER_NEW_VIDEO_OP',
    }),
  },
  methods: {
    ...mapMutations({
    ...
    playVideo(select){
      this.swiperNewVideo.forEach((item) => {
        item.play = item.id === select.id ? !item.play : false
      })
    },
  ...
  created() {
    this.newItems = this.itemNew
    this.swiperHead = this.swHead
    this.swiperHeadOp = this.swHeadOp
    this.swiperNewItems = this.swItem
    this.swiperNewItemsOp = this.swItemOp
    this.swiperNewVideo = this.video
    this.swiperNewVideoOp = this.videoOp
  },
}
</script>
