<template>
  <div class="IceHotAdmin">
    <h1 class="tytle">Ice - Hot</h1>
      ...
      <div class="newItemsTrack" v-for="(item, index) in newItems" :key="item.id">
        <div class="newItemsTrack-data">
          ...
          <div class="rating" :class="visualRating(item)">
        ...
        <div class="newItemsTrack-info">
          <div class="timeRating">{{ item.timerating }}</div>
            <a class="youTubeLinck" target="_blank" rel="noopener" :href="item.videoLinck">
...
export default {
  ...
  computed: {
    ...mapGetters({
      GET_NEW_ITEM: 'IceHot/GET_NEW_ITEM',
    }),
  },
   watch: {
    GET_NEW_ITEM(newD, lastD){
      if( JSON.stringify(newD) !== JSON.stringify(lastD))
        this.newItems = this.maxIceHost(newD)
    },
  },
  methods:{
    ...mapActions({
    ...
    maxIceHost(arr){
      let tmp = []
      arr.forEach((item, i) => i<arr.length-1?tmp.push(item):null)
      return tmp
    },
     visualRating(item){
      return item.rating>1?'ratingUp':item.rating<0?'ratingDovn':''
    },
  },
  created(){
    this.newItems = this.maxIceHost(this.GET_NEW_ITEM)
  }
}
</script>

<style lang="scss" scoped>
@import "@/assets/scss/IceHotAdmin.scss";
</style>