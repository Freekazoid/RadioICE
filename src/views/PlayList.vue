<template>
  <div class="playList">
    ...
    <div class="List">
      <div class="listRadio">
        <div v-for="(item, index) in filterList" :key="item.id" class="itemTrack" :class="sortBoxClass(index)" >
          ...
          <div class="timeTrack" :class="sortBoxClass(index)">{{ gatTime(item) }}</div>
          
          <div class="imgTrack" @click="playTrack(item, 'filterList')">
...
export default {
  ...
  computed: {
    ...mapGetters({
      GET_PLAY_LIST: 'PlayList/GET_PLAY_LIST',
      GET_SWIPER: 'PlayList/GET_SWIPER',
      GET_SWIPER_OP: 'PlayList/GET_SWIPER_OP',
    }),
    filterList(){
      let tmp = [],
        date = new Date(),
      ...
      for(const value of Object.values(this.playList)) {
        if(value.dateTime !== null){
          for(const [k, val] of Object.entries(value.dateTime)) {
            let dataTime = val.split(' '),
              chunks = dataTime[1].split(':'),
              date = new Date(),
              trackTime = { h: date.setHours(Number(chunks[0])), m: date.setMinutes(Number(chunks[1])) }

            if( value.radio === this.filter.radio && dataTime[0] === this.filter.date && (trackTime.h > filterTime.start.h && trackTime.h <= filterTime.end.h) ){
              let obj = this.filterByID(tmp, value.id)
              if(obj === undefined)
                tmp.push(value)
              else if(obj.name !== value.name && obj.artist !== value.artist)
                tmp.push(value)
            }
          }
        }
      ...
      tmp.sort((a, b) => {
        let indexA = Object.keys(a.dateTime).length-1,
          timeA = a.dateTime[indexA].split(' ')[1],
          AtimStr = timeA.split(":")[1],
          indexB = Object.keys(b.dateTime).length-1,
          timeB = b.dateTime[indexB].split(' ')[1],
          BtimStr = timeB.split(":")[1]

        return AtimStr > BtimStr ? 1 : -1
      });
      tmp.reverse()
  ...
  methods: {
    ...
    filterByID(arr, Id) {
      return arr.filter(i => i.id === Id)[0]
    },
    sortBoxClass(index){
      return index%2 ? 'right' : ''
    },
    gatTime(item){
      let chunks = []
      for(const [k, val] of Object.entries(item.dateTime)) {
        let dataTime = val.split(' ')[1]
        chunks = dataTime.split(':')
      }
      return `${chunks[0]}:${chunks[1]}`
    },
  },
  watch: {
    GET_PLAY_LIST(newD, lastD){
  ...
  mounted() {
    const dateTimeFormat = new Intl.DateTimeFormat('ru-RU', { year: 'numeric', month: '2-digit', day: '2-digit', hour: 'numeric', minute: 'numeric' }),
      [{value: month},,{value: day},,{value: year},,{value: hour},,{value: minute}] = dateTimeFormat.formatToParts(new Date()),
      hours = `${(Number(hour)-1)}`.length === 1?`0${(Number(hour)-1)}`:`${(Number(hour)-1)}`
      
    this.filter.timeStart = `${hours}:${minute}`
    this.filter.timeEnd = `${hour}:${minute}`
    this.filter.date = `${year}-${day}-${month}`
  },
  created(){
    this.playList = this.GET_PLAY_LIST
