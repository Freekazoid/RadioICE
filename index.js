  ...
  dataJson = (err=false, json=false) => {
    ...
    if(typeof json === 'object' && json.hasOwnProperty("ELEM_LIST")){
      ...
      if(typeof tableDB.liststream === 'object'){
        ...
        // } else {
        let find = tableDB.liststream.find(item => item.name === JSONFile.NAME)
        if(typeof find === 'object'){
          ...
          const timerating = addTextWeckRating(find.id),
            timeLive = JSONtime(find.id)
          ...
          log.log('info',colors.RED+'repite live '+colors.BLUE + nawDate +' '+ colors.WHITE+'id: '+find.id+' name: '+JSONFile.NAME+' === '+find.name+' artist: '+JSONFile.ARTIST+' === '+find.artist)
          // saveFile('repite live: '+nawDate+'\r\n      id: '+find.id+' name: '+JSONFile.NAME+' === '+find.name+' artist: '+JSONFile.ARTIST+' === '+find.artist+"\r\n")
          usersRespons(tableDB, 'response')
          DBupdate({liststream: {id: find.id, updatePluse: true, broadcasting: 1}})// Update table lesson from broadcasting 
          DBupdate({liststream: {id: find.id, timerating: timerating, time_live: timeLive, start_time: nowDateTime(), end_time: add3Weck()}})// Update table lesson from timerating
...
iceHost30List = () => {
  if(tableDB.hasOwnProperty('liststream')){
    ...
    if(getWeekDay() === env.START_30_ICEHOT && Array.from(tableDB.liststream)){
      tableDB.liststream.forEach(item => {
      	let daysLag = Math.ceil(Math.abs(nawDate - item.start_time.getTime()) / (1000 * 3600 * 24)),
      		end = Math.ceil(Math.abs(nawDate - item.end_time.getTime()) / (1000 * 3600 * 24))

        if(typeof date === 'undefined'){//////////////////////////////////////////
	        if(daysLag <= 7){
	        	// console.log( daysLag, end )
            	IceHot.push(item)
            }
        } 
        // else if(new Date(date.stop) !== new Date(nowDateTime("yyyy-MM-dd"))){
          // if( new Date() >=  new Date(nowDateTime(start)) && new Date(dateNow) <=  new Date(nowDateTime(end)) )
            // IceHot.push(item)
        // }
      })
      if(tmps)
